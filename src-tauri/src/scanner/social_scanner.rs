...
impl SocialScanner {
    ...
    pub fn scan(&self) -> SocialScanResultV2 {
        ...
        info!("共检测到 {} 个社交软件，{} 个扫描路径", detected_apps.len(), all_paths.len());
    ...
    fn detect_wechat_paths(&self) -> Option<Vec<SocialAppPath>> {
        ...
        if found_base_paths.is_empty() {
            ...
            if self.documents_dir != self.default_documents {
                let default_path = PathBuf::from(format!("{}\\WeChat Files", self.default_documents));
    ...
    fn scan_wechat_base_directory(&self, base_path: &Path, is_custom: bool, paths: &mut Vec<SocialAppPath>) {
    ...
    fn detect_qq_paths(&self) -> Option<Vec<SocialAppPath>> {
        ...
        if found_base_paths.is_empty() {
            ...
            if self.documents_dir != self.default_documents {
                let default_path = PathBuf::from(format!("{}\\Tencent Files", self.default_documents));
                if default_path.exists() && !found_base_paths.contains(&default_path) {
                    found_base_paths.push(default_path);
                }
            }
    ...
    fn read_qq_registry_path(&self) -> Option<String> {
        ...
        if let Ok(qq_key) = hkcu.open_subkey("Software\\Tencent\\QQ") {
            ...
            if let Ok(path) = qq_key.get_value::<String, _>("Install") {
                let tencent_files = PathBuf::from(&path).parent()
    ...
    fn scan_ntqq_directory(&self, base: &Path, paths: &mut Vec<SocialAppPath>) {
        ...
        for entry in entries.filter_map(|e| e.ok()) {
            ...
            let dir_name = sub_dir.file_name()
                .unwrap_or_default()
                .to_string_lossy()
                .to_string();
    ...
    fn scan_and_classify(&self, app_paths: &[SocialAppPath]) -> Vec<SocialCategoryStats> {
        ...
        for cat in &[
            ...
            category_map.insert(*cat, SocialCategoryStats {
    ...
    fn classify_file(&self, path: &Path, base_category: FileCategory) -> (FileCategory, RiskLevel) {
        ...
        // 数据库文件后缀
        let db_extensions = ["db", "db-wal", "db-shm", "sqlite", "sqlite-wal", "sqlite-shm"];
        ...
        // 检查是否在数据库关键目录中
        let in_critical_dir = critical_path_patterns.iter()
        ...
        let is_cache_dir = cache_path_patterns.iter()
            .any(|pattern| path_str.contains(pattern));
        ...
        let is_image_video_dir = image_video_path_patterns.iter()
            .any(|pattern| path_str.contains(pattern));
        ...
        let is_moments_dir = moments_path_patterns.iter()
            .any(|pattern| path_str.contains(pattern));
        ...
        let is_file_transfer_dir = file_transfer_path_patterns.iter()
            .any(|pattern| path_str.contains(pattern));
...
mod tests {
    ...
    fn test_file_category_risk() {
        assert_eq!(FileCategory::ChatDatabase.default_risk_level(), RiskLevel::Critical);
        assert_eq!(FileCategory::FileTransfer.default_risk_level(), RiskLevel::Medium);
        assert_eq!(FileCategory::ImageVideo.default_risk_level(), RiskLevel::Low);
        assert_eq!(FileCategory::TempCache.default_risk_level(), RiskLevel::None);
    }
}
