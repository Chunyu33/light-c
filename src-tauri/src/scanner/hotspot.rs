...
impl HotspotScanner {
    ...
    fn scan_full_disk(&self) -> Result<HotspotScanResult, String> {
        ...
        let entries: Vec<HotspotEntry> = sorted_entries
            ...
            .map(|mut entry| {
                // 对每个顶级条目执行下钻分析
                if entry.depth == 0 {
                    entry.children = Self::drill_down_directory(&PathBuf::from(&entry.path), 1, true);
                }
                entry
            })
            .collect();
            ...
            for sub_entry in sub_entries.filter_map(|e| e.ok()) {
                ...
                if sub_path.is_dir() && !Self::should_skip_scan(&sub_path) {
                    if let Some(stats) = Self::calculate_folder_stats(&sub_path) {
                        ...
                        if stats.total_size >= MIN_SIZE_THRESHOLD {
                            let folder_name = sub_path.file_name()
                                .map(|n| n.to_string_lossy().to_string())
                                .unwrap_or_default();
                            ...
                            entries.push(HotspotEntry {
                                ...
                                is_cache,
                                is_program,
                                is_safe_to_clean: !is_full_scan && is_cache && !is_program && !is_sub_protected,
    ...
    /// - 最大深度 3 层
    /// - 每层返回前 3 个最大子目录
    fn drill_down_directory(dir: &Path, current_depth: u8, is_full_scan: bool) -> Vec<HotspotEntry> {
        // 超过最大深度，停止下钻
        ...
        // 检查是否满足下钻条件
        if stats.total_size < DRILL_DOWN_SIZE_THRESHOLD || stats.file_count < DRILL_DOWN_FILE_COUNT_THRESHOLD {
            return Vec::new();
        }
        ...
        sub_dirs
            ...
            .map(|(sub_path, sub_stats)| {
                ...
                // 递归下钻子目录
                let children = Self::drill_down_directory(&sub_path, current_depth + 1, is_full_scan);
    ...
    fn is_cache_directory(path: &str, folder_name: &str) -> bool {
        ...
        let cache_keywords = [
            "cache", "caches", "tmp", "temp", "log", "logs",
            "download", "downloads", "thumb", "thumbnails",
            "crashdump", "crashreport", "backup",
        ];
