...
- **虚拟列表优化**：大量文件列表也能流畅滚动
- **深度发现**：开启后扫描所有本地固定分区；NTFS 分区使用 USN + `$MFT` 识别明确的临时、缓存和错误报告目录，非 NTFS 分区降级为受控目录遍历
- **安全规则过滤**：不按 `.tmp/.log/.cache` 全盘泛匹配，跳过回收站、系统保护目录、WebView2/Chromium 持久化数据和最近 24 小时内文件
- **多盘进度与降级**：实时显示当前分区、MFT 阶段和候选数量；MFT 不可用时仅回退到明确安全目录，并在结果中标注扫描引擎
- **按需加载结果**：深度扫描按分类分页返回，首屏限制传输量，展开分类后再加载后续文件，避免大盘缓存结果一次性占满前端内存
- **深度删除复核**：删除深度结果前由 Rust 后端重新校验路径规则，多分区物理释放空间按所在卷簇大小计算
- **删除后真实同步**：深度分类当前页全选时，清理会覆盖该分类的完整扫描结果；删除完成后统一真实重扫并刷新分类数量和占用空间，失败条目不会从界面假消失
- **大批量删除进度**：清理时显示已处理文件数、释放空间、失败数、处理速度和预计剩余时间；删除命令完成后立即解除遮罩，扫描结果在后台核验并自动同步
- **整类选择语义**：分类勾选会明确记录完整分类选择，手动取消单项会自动转为部分选择，避免分页结果下误删或漏删
- **文件用途搜索**：垃圾文件列表支持复用统一搜索引擎查询文件用途，搜索词和列表展示会自动去除 Windows `\\?\` 长路径前缀
...
### 🖥️ 虚拟磁盘管理
- **外壳节点扫描**：扫描 Explorer「此电脑」下 `MyComputer\NameSpace` 的第三方 CLSID 外壳图标，并根据 CLSID、组件文件和注册表描述关联到百度网盘、夸克网盘、WPS、迅雷等应用名称；同一 CLSID 在不同 Hive 或 Registry View 下的节点分别展示，确保不会漏处理复活来源。
- **系统节点过滤**：固定白名单、Windows/Microsoft 系统组件、RegFolder 内部节点和无法确认归属的节点直接过滤，不占用用户列表。
- **双操作入口**：`删除` 会备份后物理移除当前节点，软件可能再次创建；`彻底删除` 会备份、物理移除 Namespace 节点，再锁定父级 Namespace 的创建权限防止普通用户复活，并自动刷新 Explorer；父级 ACL 会随备份保存，恢复时一并还原，旧版遗留的空锁定节点仍可重新处理。
- **备份去重**：同一 Hive、Registry View 和 CLSID 只保留一份原始 `.reg` 与元数据备份，兼容旧版本带时间戳的备份文件。
- **可恢复操作**：每次操作保存 `.reg` 和原始 ACL 元数据；兼容 LightC 旧版防复活 ACL，普通删除会先解除 LightC 旧锁并物理删除节点，彻底删除会额外锁定父级 Namespace 的创建权限；恢复时先还原父级 ACL，再导入原始节点。
- **快速定位**：支持一键打开注册表编辑器并定位到对应的 `NameSpace\{CLSID}` 节点。
...
- **版权与渠道声明**：首页底部及设置中明确标注官方发布渠道（GitHub Releases、当前 Release 的 `download.json` 网盘链接及内置兜底、B站/抖音 @Evan的像素空间），并提示第三方转载、网盘引流和二次打包风险
