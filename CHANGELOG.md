...
- **应用关联展示**：根据 CLSID、注册表描述和组件路径识别百度网盘、夸克网盘、WPS、迅雷等应用，不再只显示难以理解的 CLSID 名称。
- **结果降噪**：过滤系统白名单、`%SystemRoot%\System32`/`SysWOW64` 组件、RegFolder 内部节点、未知归属项；不同 Hive 或 Registry View 的同 CLSID 节点分别保留，避免漏掉实际复活来源。
- **双模式清理**：保留“删除”和“彻底删除”两个入口；彻底删除执行物理移除并锁定父级 Namespace，普通删除不改变父级权限。
- **安全分类**：清理命令会在后端重新校验目标，不能通过前端伪造注册表路径。
- **可恢复清理**：操作前自动保存 `.reg` 和 ACL 元数据；恢复时兼容目标键及父级 Namespace 权限，支持解锁和从最近备份恢复。
- **Explorer 刷新**：提供显式刷新 Explorer 和打开备份目录入口，不在每次操作后自动重启桌面进程。
- **注册表定位**：新增一键打开 Regedit 并定位到外壳节点的入口。
- **拒绝访问修复**：兼容百度网盘借用 `shdocvw.dll` 系统宿主的 Namespace 节点；正确识别 LightC 旧版 `DCLCSD` 拒绝 ACE，清理前仅移除 LightC 自己的锁定规则后再执行备份和防复活。
- **彻底删除与定位修复**：新增“彻底删除”入口，先备份并移除 LightC 自己的旧锁，再物理删除 Namespace 节点并锁定父级创建权限；定位前关闭旧 Regedit 进程，确保 `LastKey` 被读取并展开到 CLSID 节点。
- **操作入口收敛**：用户侧仅保留“删除”和“彻底删除”；同一节点备份改为稳定文件名并自动合并旧版重复备份。
- **删除结果核验**：普通删除和彻底删除都会确认目标注册表节点物理不存在；彻底删除还会确认父级防复活 ACL 已生效，失败时不报告成功。
- **Explorer 缓存处理修复**：彻底清理成功后由后端自动重启 Explorer，避免注册表已修改但此电脑仍显示旧图标；旧版遗留的空锁定节点会继续显示操作入口，可重新执行物理删除。
- **防复活策略重构**：彻底删除不再保留 Explorer 可枚举的空占位键，改为物理删除目标 Namespace 节点并锁定父级创建权限；父级 ACL 纳入备份和恢复，避免出现“注册表已清空但图标仍可见”的假成功。
- **恢复兼容修复**：解锁和恢复操作现在同时处理父级 Namespace ACL；旧版没有父级 ACL 快照时，会只移除 LightC 自己的旧拒绝规则，避免恢复后仍无法重新注册。
